...
<script setup lang="ts">
	...
	const userMode = props.kind === "user";
	...
	const allAuthorizedMenus = ref<TreeNodeData[]>([]);
	...
	const onUpdatingFormData: UpdatingFormDataHandler = async function (rawData, base, modalDialogProps) {
		...
		const params = { id: rawData!["id"], all: false };
		if (allAuthorizedMenus.value.length == 0) {
			params.all = true;
		}
	...
	function getIndependentCheckedAmount(prev: number, cur: Node) {
		...
		if (!cur.disabled && checked && ((data.independent && (cur.isLeaf || data.independent.checkedSubAmount != 0)) || (cur.isLeaf && !cur.data["origin"]))) {
			++prev;
		}
		return prev;
	}
				...
				if (element.hasAttribute("changed")) {
					if (element.hasAttribute("independent")) {
						if (
							...
							(data.independent!.curSubAmount == data.independent!.totalSubAmount && !element.querySelector(":scope [changed] .icon-user-line"))
						) {
							element.removeAttribute("independent");
